...
const fields = [{
  ...
  name: "password", label: "گذرواژه", rules: {
    required: "این فیلد اجباری است", minLength: {
      value: 6, message: "حداقل طول رمز عبور ۶ کاراکتر است"
    }
  }
}];
const LoginForm = () => {
  ...
  return (<div>
    {fields.map((field, idx) => (<Controller
      key={field.name}
      control={control}
      name={field.name}
      rules={field.rules}
